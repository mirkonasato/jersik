...
//
package io.encoded.jersik.maven
...
import io.encoded.jersik.java.codegen.JavaCodeGenerator
...
class SchemaToCodeMojo extends AbstractMojo {
  ...
  @Parameter(defaultValue = "${project.build.directory}/generated-sources/jersik/java")
  ...
  override def execute(): Unit = {
    log.info("Generating Jersik Java sources in "+ targetPath)
    ...
    for (inputFile <- inputFiles) {
      ...
      JavaCodeGenerator.generateModule(inputFile, targetPath, generateClient, generateServer)
    }
